...
impl Contract {
    ...
    pub fn buy(
        ...
        let mut market_data: MarketData = market_data.expect("Marble: Market data does not exist");
        
        assert!(market_data.pending_status == false, "Marble: This NFT is already on pending.");

        assert_ne!(
            buyer_id.clone(), market_data.owner_id,
        ...
        market_data.pending_status = true;
        ...
        market_data.buyer_id = Some(buyer_id);
    ...
    pub fn add_bid(
        ...
        assert!(market_data.pending_status == false, "Marble: Token is on shipping");
    ...
    pub fn accept_bid(&mut self, nft_contract_id: AccountId, token_id: TokenId) {
        ...
        market_data.bids = Some(bids);
        market_data.pending_status = true;
        market_data.buyer_id = Some(selected_bid.bidder_id.clone());
        market_data.price = selected_bid.price.clone().0;
        self.market.insert(&contract_and_token_id, &market_data);
    ...
    fn internal_delete_market_data(
        ...
        let market_data: Option<MarketData> = if let Some(market_data) = self.market.get(&contract_and_token_id) {
    ...
    #[payable]
    pub fn finish_sale(&mut self, nft_contract_id: AccountId, token_id: TokenId) {
        assert_one_yocto();
        let contract_and_token_id = format!("{}{}{}", nft_contract_id, DELIMETER, token_id);
        let market_data = self.market.get(&contract_and_token_id).expect("Marble: Market data does not exist");
        assert!(market_data.pending_status == true, "Marble: This token is not on shipping");
        assert!(env::predecessor_account_id() == self.owner_id.clone(), "Marble: Unauthorized");
        let buyer_id = market_data.buyer_id.expect("Marble: Buyer Id should be defined");
    ...
    #[payable]
    pub fn refund_sale(&mut self, nft_contract_id: AccountId, token_id: TokenId) {
        assert_one_yocto();
        let contract_and_token_id = format!("{}{}{}", nft_contract_id, DELIMETER, token_id);
        let market_data = self.market.get(&contract_and_token_id).expect("Marble: Market data does not exist");
        assert!(market_data.pending_status == true, "Marble: This token is not on shipping");
        ...
        let buyer_id = market_data.buyer_id.expect("Marble: Buyer Id should be defined");
        if market_data.ft_token_id == near_account() {
            Promise::new(buyer_id.clone()).transfer(market_data.price);
        } else {
            ext_fungible_token::ft_transfer(
                buyer_id.clone(),
    ...
    pub fn get_market_data(self, nft_contract_id: AccountId, token_id: TokenId) -> MarketDataJson {
        ...
        MarketDataJson {
            ...
            pending_status: market_data.pending_status,
            buyer_id: market_data.buyer_id
        }
    }
